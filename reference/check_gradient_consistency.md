...
the objective, such as one holding a predictor fixed where the objective
...
from the truncation error of the difference it is compared with. That
last case is the one a caller who starts at the optimum supplies, and
firing there would penalize the best possible start.
...
objective does not trip it. It exists to catch the wrong function, not
the eighth digit.
