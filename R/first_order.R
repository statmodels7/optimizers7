...
#' criticism of it, because steepest is a statement about an arbitrary choice of
#' length. Consecutive directions under an exact line search are orthogonal, so
#' on a narrow valley the iterates zigzag across the floor and progress along it
#' only slowly. \code{\link{cg}} repairs exactly this by bending each direction
#' back towards the last, at no extra cost per iteration.
#'
#' Reach for it when you want the simplest thing that works, or as the baseline
#' another method has to beat. On anything smooth \code{\link{bfgs}} will beat
...
#'   \code{1}.
#' @param line_search Defaults to \code{wolfe(c2 = 0.1)}, and see Details —
#'   neither the search nor that constant is a free choice.
...
#' steepest descent; it can fail to converge in theory, and the standard repair
#' is to clamp \eqn{\beta} at zero, which is what this does. The clamp is not a
#' fudge — \eqn{\beta = 0} \emph{is} a restart, and it is reported as one in the
#' trace. \code{"hs"} and \code{"dy"} are the other two standard choices.
#' }
#'
#' \subsection{The line search is not optional, and neither is its tolerance}{
...
#'
#' @description
#' Gradient descent with one number changed, and the number is what makes it
#' work: the step length is a curvature estimate taken from the previous step
...
#' switches between them at each iteration, which is the usual recommendation and
#' the default: they estimate the same curvature from different ends, and taking
#' them in turn is more robust than trusting either.
#' }
#'
