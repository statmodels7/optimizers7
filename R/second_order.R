...
#'   `1`, which is the natural Newton step and is accepted unchanged near
#'   the solution.
#' @param line_search See [armijo()] and [wolfe()].
...
#' whose size is set by `floor` rather than by any curvature of the
#' objective: the direction is therefore scaled to
#' \eqn{\min(1, 1/\lVert d\rVert_\infty)}, a displacement of order one in the
#' parameters. This is the rule [bb()] applies when its secant pair
#' reports no curvature and the one [bfgs()] and [lbfgs()]
...
#' as `"hessian modified (capped)"`. The same scaling is applied to the
...
#' [minimize()], the Hessian is obtained by differencing the gradient.
#' That is one numerical differentiation when the gradient is analytic and
#' acceptable; when the gradient is *also* differenced it is two composed,
...
#' by [bfgs()], which never needs a Hessian at all.
#'
#' @return An S7 object of class `Newton`.
#'
#' @examples
...
#' @description The class [bfgs()] instantiates.
...
#' @param step,line_search As in [bfgs()].
#' @return An S7 object inheriting from [optimizer()].
#' @seealso [bfgs()]
#' @name Bfgs-class
#' @aliases Bfgs
...
#'   [newton()]. The default line search is [wolfe()]; see
#'   Details.
#'
#' @details
...
#' `max_skip` consecutive skips there is no curvature information left worth
...
#'
#' @return An S7 object of class `Bfgs`.
#'
#' @examples
#' rosen <- function(p) (1 - p[1])^2 + 100 * (p[2] - p[1]^2)^2
#' minimize(bfgs(), rosen, c(-1.2, 1))
#'
#' @seealso [lbfgs()] for many parameters, [newton()] when a
...
#' a handful of parameters that is no gain whatever and [bfgs()] will
...
#'
#' @return An S7 object of class `Lbfgs`.
#'
#' @examples
#' rosen <- function(p) (1 - p[1])^2 + 100 * (p[2] - p[1]^2)^2
